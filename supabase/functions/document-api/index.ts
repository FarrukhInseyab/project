...
async function getCustomerData(customerNo: string | number) {
...
async function getTemplateMappings(templateId: string, templateVersion: number) {
  const { data, error } = await supabase
    .from("data_mappings")
    .select("*")
...
async function updateCustomerStatus(customerNo: string | number, status: string) {
  // Convert to number for proper comparison with bigint column
  const customerNoInt = typeof customerNo === "string" ? parseInt(customerNo, 10) : Number(customerNo);
...
async function downloadTemplateFile(storagePath: string) {
...
async function createGenerationRecord(
  ...
  const { data, error } = await supabase
    ...
    .insert({
      ...
      file_urls: [], // Will be updated after upload
      status: "processing",
      metadata: {
        source: "api",
        output_format: outputFormat,
        ...(customerNo && { podapi_customer_nos: [String(customerNo)] })
      }
    })
    .select("*")
    .single();
...
async function generateDocument(templateBuffer: ArrayBuffer, data: any, tags: any[], mappings: any[] = []) {
  try {
    ...
    if (mappings && mappings.length > 0) {
      ...
      tags.forEach(tag => {
        ...
        // Then try with common variations
...
async function convertDocxToPdfWithOnlyOffice(docxBuffer: Uint8Array, filename: string, serverUrl: string): Promise<Uint8Array> {
...
async function convertDocxToPdf(docxBuffer: Uint8Array, filename: string, userId: string): Promise<Uint8Array> {
...
function convertDocxToText(docxBuffer: Uint8Array, mappingsObj: Record<string, string>): string {
  try {
...
serve(async (req) => {
  ...
  try {
    ...
    if (path === "generate-document" && req.method === "POST") {
      ...
      if (!["docx", "pdf", "text"].includes(outputFormat)) {
      ...
      if (outputFormat === "docx") {
        ...
        const docxStoragePath = await uploadGeneratedDocument(
          docxBuffer,
          user.id,
          generationRecord.id,
          outputFilename,
        ...
        try {
          ...
          const pdfStoragePath = await uploadGeneratedDocument(
            pdfBuffer,
            user.id,
            generationRecord.id,
            outputFilename,
          ...
          await updateGenerationRecord(
            generationRecord.id,
            [docxStoragePath, pdfStoragePath],
            "completed"
          );
          ...
          await updateGenerationRecord(
            ...
            [],
            "failed",
            pdfError instanceof Error ? pdfError.message : "Unknown PDF conversion error"
          );
      ...
      return new Response(JSON.stringify({ data: customerData }), {
        status: 200,
        headers: {
          ...corsHeaders,
          "Content-Type": "application/json",
      ...
      const processedData = data.map(template => {
        // Extract tag names
        const tagNames = template.tags ? template.tags.map((tag: any) => tag.name) : [];
        const tagDisplayNames = template.tags ? template.tags.map((tag: any) => tag.display_name) : [];
      ...
      const { 
        templateId, 
        jsonData, 
        outputType = "docx" // docx, pdf, or text
      } = requestData;
      ...
      if (!["docx", "pdf", "text"].includes(outputType)) {
