/*
  # Document AI Studio - Complete Database Schema

  This migration creates the entire database schema for the Document AI Studio application,
  including all tables, functions, triggers, policies, and storage buckets.

  1. Core Tables
    - `profiles` - User profiles and preferences
    - `template_categories` - Template organization categories
    - `document_templates` - Reusable document templates with full-text search
    ...
    - `template_sharing` - Template collaboration and sharing
    - `ProofofDebitAPI` - External API integration for customer data
  ...
  3. Storage
    - document-templates bucket for original DOCX files
    - generated-documents bucket for generated files
...
-- Add unique constraint for template_version
ALTER TABLE data_mappings ADD CONSTRAINT data_mappings_template_version_unique 
...
-- Function to debug all mappings
...
-- Create full-text search trigger for templates
