/*
  ...
  3. Functions
    ...
    - `update_template_search_vector`: Updates search vector for full-text search
...
-- Create trigger function to create initial template version
...
USING (template_id IN (SELECT id FROM document_templates WHERE user_id = uid()))
WITH CHECK (template_id IN (SELECT id FROM document_templates WHERE user_id = uid()));
...
BEGIN
    ...
    -- Get old file size for metadata
    SELECT file_size INTO v_old_file_size
    ...
    SELECT
        ...
        jsonb_build_object(
            ...
            'file_size_change', p_file_size - v_old_file_size
        )
...
BEGIN
    ...
    SET 
        ...
        metadata = jsonb_set(
            coalesce(metadata, '{}'::jsonb),
            '{rollback_notes}',
            to_jsonb(coalesce(p_rollback_notes, 'Rollback to previous version'))
        )
